...
import { logLoginFailure } from '../utils/http.js'
...
export async function signup(req, res) {
  ...
  if (role === 'VENDOR') {
    return res.status(403).json({
      error: 'Vendor accounts must be created by an administrator.',
    })
  }
...
export async function login(req, res) {
  ...
  try {
    ...
    const maxAgeMs = 7 * 24 * 60 * 60 * 1000
    res.cookie(COOKIE_NAME, token, sessionCookieOptions(maxAgeMs))
